...
class Sheild(Scene):
    def construct(self):
        ...
        for i in range(n):
            start_angle = -(deg[i] + angle_per_slice+180) * DEGREES
            ...
            labels.add(label)
            path.add(Arc(stroke_opacity=0, color = c[i], radius=r, start_angle=(54+72*i)*DEGREES, angle=72*DEGREES, arc_center=[0,0,0]))
        ...
        for i in range(n):
            ...
            self.wait()
            if i==1:
                new_shield = Text("Shield : 4", font_size=24).move_to(shield.get_center())
                ...
                new_shield = Text("Shield : 1", font_size=24).move_to(shield.get_center())
                new_score = Text("Score : 3", font_size=24).move_to(score.get_center())
                new_log = Text("Player jumps from Sector 2 to Sector 4", font_size=24).move_to(log.get_center())
                ...
                self.play(
                    human.animate.move_to([pos[3][0]*k,pos[3][1]*k,0]),
                )
            else:
